...
namespace SimpleDemos
{
    ...
    public class SendFloatArray_Example : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (function == Function.Jump) 
            {
                m_ObjectToSendFloats.GetComponent<ASL.ASLObject>()._LocallySetFloatCallback(MyJumpFunction);
            }
        }
        ...
        * https://uwb-arsandbox.github.io/ASL/ASLDocumentation/Help/html/5a174bc7-f1fe-c49a-b81d-4ad397ffb286.htm Position*/
        ...
        public static void MyJumpFunction(string _id, float[] _myFloats)
        {
            ASL.ASLObject tempASLObject;

            if (_myFloats[0] >= 1.0f)
            {
                ASL.ASLHelper.m_ASLObjects.TryGetValue(_id, out tempASLObject);
                tempASLObject.gameObject.GetComponent<Rigidbody>().AddForce(new Vector3(0, 1000 * tempASLObject.GetComponent<SimpleDemos.SendFloatArray_Example>().m_MyFloats[4], 0));
                tempASLObject.GetComponent<SimpleDemos.SendFloatArray_Example>().m_MyFloats[0] = 0;
            }
